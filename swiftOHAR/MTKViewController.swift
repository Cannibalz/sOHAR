...
class Renderer : NSObject, MTKViewDelegate
{
    ...
    let sampler: MTLSamplerState    //採樣器
    let texture: MTLTexture         //材質
    let mesh: Mesh
    ...
    init?(mtkView: MTKView) {
        ...
        commandQueue = device.makeCommandQueue()    //使用GPU
        ...
        mesh = Mesh(cubeWithSize: 1.0, device: device)! //方塊大小
        
        do {
            //texture = try Renderer.buildTexture(name: "checkerboard", device)
            texture = try Renderer.buildTexture(name: "Test", device)
        }
        catch {
