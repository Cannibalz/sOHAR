...
class Mesh {
    ...
    init?(cubeWithSize size: Float, device: MTLDevice)
    {
        ...
        let mdlMesh = MDLMesh(boxWithExtent: vector_float3(size, size, size*2),
                              segments: vector_uint3(10, 10, 10),
                              inwardNormals: false,
                              geometryType: .triangles,
                              allocator: allocator)
        var mdlMM = MDLMesh(sphereWithExtent: vector_float3(size, size, size*2), segments: vector_uint2(10, 10), inwardNormals: false, geometryType: .triangles, allocator: allocator)

        do {
            let mtkMesh = try MTKMesh(mesh: mdlMM, device: device)
