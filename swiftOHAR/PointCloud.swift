...
@objc class PointCloud: NSObject {
        ...
        if let path = Bundle.main.path(forResource: file, ofType: "txt") {
            do {
                ...
                for i in 0...(self.n-1) {
                    ...
                    pointCloud[i].x = CGFloat(x)
                    pointCloud[i].y = CGFloat(y)
                    pointCloud[i].z = CGFloat(z)
                }
    ...
    public func getNode() -> SCNNode {
        ...
        for i in 0...(points.count-1) {
            ...
            vertices[i].r = Float(arc4random()) / Float(UINT32_MAX)//Float(0.0)
            vertices[i].g = Float(arc4random()) / Float(UINT32_MAX)//Float(1.0)
            vertices[i].b = Float(arc4random()) / Float(UINT32_MAX)//Float(1.0)
        }
